...
The workflow reads `capabilities/<capability-slug>/<capability-slug>.md` and creates `solution-architectures/<slug>/<slug>-architecture.md` using the skill template and chapter-level writing guidance. When Mermaid diagrams are created, they are stored as separate `.mmd` source files in the same `solution-architectures/<slug>/` folder, rendered to same-basename `.svg` files, and embedded in the architecture document. When Draw.io diagrams are requested, use `../create-drawio-diagram/SKILL.md` to create editable `.drawio` files from reusable diagram templates.
