...
The workflow reads `capabilities/<capability-slug>/<capability-slug>.md` and creates `solution-architectures/<slug>/<slug>-architecture.md` using the skill template and chapter-level writing guidance. Diagrams are created as editable Draw.io `.drawio` files from reusable diagram templates and exported to same-basename `.svg` files when embedded.
