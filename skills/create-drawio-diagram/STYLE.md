...
When a component, capability, system, or dependency is implemented by a named application, place only the application name in a separate 10 px high header box overlaid on the top edge of the component. Use the same `x`, `y`, and width as the component, set the header height to 10 px, and render the header above the component so it floats over the component body instead of sitting above it. Use `fillColor=#000000`, `strokeColor=#000000`, `fontColor=#ffffff`, `fontStyle=1`, `fontSize=6`, `spacing=0`, and `strokeWidth=2`. Put capability names, component names, responsibilities, dependencies, and integration roles in the component body. If the application name is unknown, omit the header.
...
- Canvas size is flexible. Increase `pageWidth` and `pageHeight` whenever the default template size would force cramped components, overlapping connectors, clipped labels, or crowded layer bands. For layered diagrams, widen every layer band consistently so the layers stay aligned.
...
- Treat every component body, border, application header, label, icon, badge, and nested child shape as blocked space. Do not route connectors through, over, or behind any of them. The only header exception is a deliberate endpoint on the composite component's outer top-center boundary.
...
- Treat an application-name header and the component body as one composite shape. A connector may attach to the composite's outer top-center boundary when the first or last segment travels straight outward into whitespace. The connector must not enter, cross, follow, or hide behind the header interior. Use an explicit left, right, or unobstructed bottom port when top-center attachment would create crowding or an unclear arrowhead.
- Do not rely on z-order to hide a collision. A connector concealed behind a component is still an invalid route.
- For cross-layer flow connectors, prefer top-to-bottom vertical orthogonal lines when the components align. An application header may use its explicit outer top-center boundary port; otherwise use a side or bottom port and horizontal jogs so the connector never crosses the header interior.
