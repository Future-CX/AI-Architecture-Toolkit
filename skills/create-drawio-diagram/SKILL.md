...
- Treat the complete visible component as blocked space. This includes its body, border, text, icon, application-name header, badge, and any nested child shape. The only header exception is a deliberate endpoint on the composite component's outer top-center boundary.
...
- Treat an application-name header and its body as one composite component. A connector may attach at the composite's outer top-center boundary when it immediately travels outward into whitespace and does not enter the header interior. Never route a connector across, along, through, or underneath the header. Use a left, right, or unobstructed bottom port when the top-center port would create crowding, overlap, or an unclear arrowhead.
...
- Keep horizontal provider and consumer routing lanes outside the full middle zone occupied by actors, the target capability, and constraints. A tall actor stack must not extend into these lanes. Prefer the explicit outer top-center port for providers with application headers, then route through whitespace lanes; use side ports when the top-center route would be crowded or ambiguous.
...
The helper uses one shared layout calculation for `.drawio` and `.svg`: centered rows, ordered connector bends, distinct ports, top-center boundary exits from application headers, and a canvas fitted to the content. Preserve these rules when editing a generated diagram manually. Replace or remove every template placeholder, including application headers, before accepting the output.
...
- When the solution includes a Backend-for-Frontend component, place it in the Frontend layer directly below the frontend/channel component it supports. Align the frontend component and BFF on the same x-position. Prefer a vertical connector from the lower component's explicit outer top-center boundary to the upper component's bottom-center boundary. Use side ports in adjacent whitespace when the vertical route would be crowded or ambiguous.
...
- Use explicit entry and exit ports plus all required orthogonal waypoints. Set `noEdgeStyle=1;exitPerimeter=0;entryPerimeter=0;` on these edges so the editor preserves the planned route instead of choosing new bends automatically. A top-center endpoint on an application header must be explicit and must travel immediately outward; otherwise use a side or bottom port.
...
- Place components inside the corresponding layer band, ordered from top to bottom: Public Internet, Frontend, Engagement Services, Integration, and Enterprise Foundation (Backoffice).
- Always place the Backend-for-Frontend directly below the New Webshop component, aligned on the same x-position. Keep both components inside the Frontend layer. Prefer a vertical connector from the lower component's explicit outer top-center boundary to the upper component's bottom-center boundary; otherwise route through side ports in the adjacent whitespace. Do not place the BFF in the Engagement Services or Integration layer, even when it calls APIs, composes requests, or shapes channel responses.
...
- Place a Backend-for-Frontend directly below the frontend/channel participant it supports and align it on the same x-position. Prefer a vertical connector from the lower component's explicit outer top-center boundary to the upper component's bottom-center boundary; otherwise route through side ports in the adjacent whitespace. Do not place the BFF as a peer beside the frontend component unless the flow has multiple frontend participants and vertical placement would make the path unreadable.
