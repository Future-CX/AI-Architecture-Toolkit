...
Write business capability overview documents under the consuming repository's root `capabilities/<slug>/` folder using a preset markdown template. The helper also creates an editable `diagrams/<slug>-capability-overview.drawio` context diagram, exports `diagrams/<slug>-capability-overview.svg`, and embeds the SVG in the generated overview.
