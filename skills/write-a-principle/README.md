...
Generated principle documents include a linked list at the top of the `## Principles` section. Each item contains the principle identifier and name, and links to the matching principle detail section below. Identifiers use a 3-4 letter category code plus a three-digit incremental number starting with `001`, such as `DATA001`. Each principle detail includes identifier, name, description, rationale, consequences, and optional do's and don'ts.
