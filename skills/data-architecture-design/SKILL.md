...
- Always place the Backend-for-Frontend directly below the New Webshop component. Keep both components inside the Frontend layer and align them on the same x-position. Prefer a vertical connector from the lower component's explicit outer top-center boundary to the upper component's bottom-center boundary. Use side ports in adjacent whitespace when the vertical route would be crowded or ambiguous.
...
- Route every connector through whitespace between components. A connector may touch its source and target only at deliberate boundary ports; every other segment must stay at least 10 px from component bodies, application headers, layer labels, and other connector labels. A connector may attach to the outer top-center boundary of a composite component with an application-name header only when it immediately travels outward and never enters the header interior. Move components, enlarge layer bands, choose an unobstructed side or bottom port, and add explicit orthogonal waypoints until the complete rendered route is clear.
